...
# Simulating a live catalogue lookup for Local Commerce
INVENTORY_DB = {
...
3. When telling the user a price, you MUST state when the data is from based on the tool's output (e.g., "আজকের আপডেট অনুযায়ী..." / "According to today's update...").
4. GRACEFUL FAILURE: If the `check_kirana_inventory` tool returns an error or says the system is down, do not invent prices. Apologize and say exactly: "ক্ষমা করবেন, সার্ভারে সমস্যার জন্য আমি এই মুহূর্তে স্টকের তথ্য দেখতে পাচ্ছি না। আপনি কি অন্য কিছু খুঁজছেন?"
...
- Always write every language in its own native script. Bangla → Bengali script (নমস্কার).
...
class Assistant(Agent):
    ...
    async def check_kirana_inventory(self, context: RunContext, item_name: str, simulate_failure: bool = False):
        ...
        # Simulating an API timeout or server crash
        if simulate_failure:
...
async def my_agent(ctx: JobContext):
    ...
    # Force Mita to speak her greeting immediately upon connection

...
# --- SQLITE DATABASE SETUP (Day 4 & Day 7 Memory) ---
def init_db():
...
4. Ask for permission before saving order details to memory.

DAY 7 ESCALATION RULES:
5. WHEN TO ESCALATE: If the caller has a payment issue, a refund dispute, or explicitly asks to speak to a human/manager, you must escalate the call.
6. PERMISSION FIRST: Before calling the `create_escalation` tool, you MUST explicitly ask the user for permission to send their details to a human team member. If they say no, apologize and do not use the tool.
7. SUMMARY: Gather who needs help, what happened, what you checked, how urgent it is, and their language to pass into the tool.
8. THE HANDOFF: After the `create_escalation` tool runs, give the user the generated Reference ID and clearly state that a human support agent will review the ticket and contact them shortly. Do not promise an immediate reply.
...
class Assistant(Agent):
    ...
    async def create_escalation(self, context: RunContext, summary: str, urgency: str):
        ...
        Args:
            summary: A brief summary detailing who needs help, what happened, what you already checked, and the caller's language. Do NOT include passwords or sensitive data.
            urgency: 'High' (for payment/refund issues) or 'Normal'.
        """
        ref_id = f"ESC-{uuid.uuid4().hex[:4].upper()}"
        
        # Save the ticket to the local SQLite database
        conn = sqlite3.connect('home_fresh.db')
        c = conn.cursor()
        c.execute('''
        ...
        # Log it so it acts as a real-time dashboard notification in the terminal
